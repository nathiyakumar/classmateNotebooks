...
const getSingleShippingZone = graphql`
    query ShippingZonePageGetSingleShippingZoneQuery($id:ID!){
         shippingZone(id:$id){
           name
            countries{
              code
...
class ShippingZonePage extends React.Component{
    ...
    componentWillMount(){
        this.getSingleShippingZone();
    };
    getSingleShippingZone = () => {

        let variables = {
          id : this.props.match.params.id
        };

        fetchQuery(environment, getSingleShippingZone, variables)
            .then(data => {
                if(data.shippingZone !== null ){
                    let shipping_Zone_form_data = {...this.state.shipping_Zone_form_data};

                    shipping_Zone_form_data['ShippingMethodList'] = data.shippingZone.shippingMethods;
                    shipping_Zone_form_data['name'] = data.shippingZone.name;
    ...
    handleSubmit = () => {
        ...
        //     })
        // }
        // if(this.state.shipping_form_data.shippingZone === {}){
        //     error = {
        //         field: "shippingZone",
        ...
        //         price:shipping_form_data.seoTitle,
            ...
            <div>
                <AdminDashboardIndex active_page={"configuration"} >
                    <LoadingScreen
                        ...
                        text="Loading"
                    >
                        <div className="shipping_zone_component">
                            <form>
                                <Container maxWidth={"lg"} className={"shipping_zone_container"}>
                                    <AppHeader  title={"SHIPPING"} onBack={this.GoBack}/>
                                    <PageHeader title={this.state.shipping_Zone_form_data.name}/>
                                    <Grid>
                                        {
                                            this.state.get_response === true && (
                                        <ShippingZoneDetails
                                            data={this.state.shipping_Zone_form_data}
                                            goCreatePath={this.goToAddPath}
                                            action="add"
