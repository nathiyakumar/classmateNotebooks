...
import {addNotebookImages, SetAllCanvas} from '../../../Actions/index'
...
const Test_notebook = 'https://cdn.classmateshop.co.in/live/Front-Assets/FrontEnd/defalut_notebook.png';
...
class Canvas extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            userSelectedImages: [],
            FormattedPreviewImages: [],
            canvas: [],
            currentIndex: 0,
            translateValue: 0,
            panel1_expanded: true,
            panel2_expanded: true,
    ...
    componentWillMount() {
        ...
        this.setState({
            ...
            previewImage: this.props.UserSelectedImages[this.props.CurrentIndex]['preview_url'],
            rmvClass: 'rmv',
            loading: false,
            previewImages: this.props.converted_images
        });
    }
        ...
        document.body.addEventListener("keydown", function (e) {
            if (e.which === 46) {
                ...
                if (activeObject && activeObject.type === 'text') {
                    ...
                    activeObject.text =new_msg
                }
                if (e.which === 8) {
                    // let canvas = this.state.canvas;

                    if (activeObject) {
                        this.updateCanvas(activeObject)
                    }
    ...
    goToPrevSlide = () => {
        if (this.state.currentIndex === 0) {
            this.setState({
                ...
                activeItemIndex: this.props.selected_quantity - 1,
                translateValue: (this.props.selected_quantity - 1) * (-(this.slideWidth())),
                selectedThumbImage: this.state.userSelectedImages[this.props.selected_quantity - 1]['preview_url']
            })
        } else {
            this.setState(prevState => ({
                currentIndex: prevState.currentIndex - 1,
                activeItemIndex: prevState.currentIndex - 1,
                translateValue: prevState.translateValue + this.slideWidth(),
                previewImage: this.state.userSelectedImages[prevState.currentIndex - 1]['preview_url']
    ...
    goToNextSlide = () => {
        if (this.state.currentIndex === this.state.canvas.length - 1) {
            return this.setState({
                ...
                activeItemIndex: 0,
                translateValue: 0,
                previewImage: this.state.userSelectedImages[0]['preview_url']
            })
        }
    ...
    toggleExpansionPanel = panel => {
        if (panel === 'panel1') {
            this.setState({
                panel1_expanded: !this.state.panel1_expanded
            })
        }
        if (panel === 'panel2') {
            this.setState({
                panel2_expanded: !this.state.panel2_expanded
            })
        }
    };
    ...
    setReplacedImageInCanvas = (current_index) => {
        ...
        this.setState({
            canvas: canvas,
            userSelectedImages: user_selected_images,
            FormattedPreviewImages: this.listToMatrix(user_selected_images, 6),
            previewImage: user_selected_images[current_index].preview_url,
            loading: false
        ...
        setTimeout(
            function () {
                ...
                previewImages[current_index].preview_url = this.state.canvas[current_index].toDataURL();
                this.setState({
    ...
    removePrevieImage = () => {
        ...
        this.setState({
            ...
            let user_selected_images = this.state.userSelectedImages;
            let defalut_notebook = {
                'preview_url': Test_notebook,
                'image_name': 'emptyImage',
                'is_empty_image': true
            };
            user_selected_images[this.state.currentIndex] = defalut_notebook;
            ...
            canvas[this.state.currentIndex]['_objects'][0]._element.src = Test_notebook;
            let scope = this;
            setTimeout(function () {
                canvas[scope.state.currentIndex].renderAll();
            }, 50);
            this.setState({
                canvas: canvas,
                userSelectedImages: user_selected_images,
    ...
    addTextToCanvas = () => {
        if (this.state.canvas_text !== "") {
            ...
            let scope = this;
            // let newText = new fabric.IText(this.state.canvas_text, { left: 40, top: 100,fontFamily: this.state.fonts[0] });
            let newText = new fabric.Textbox(this.state.canvas_text, {
                width: canvas[this.state.currentIndex].width, lockUniScaling: true,
                breakWords: true, top: 100, fontFamily: this.state.fonts[0]
            });
            newText.set({
                text_id: count,
    ...
    setTextAttributes = (current_index, value) => {
        ...
        if (value === "select") {
            ...
            is_text_bold = textObject.__dimensionAffectingProps.fontWeight === "bold";
            is_text_italic = textObject.__dimensionAffectingProps.fontStyle === "italic";
            is_text_underline = textObject.underline === "underline";
    ...
    convertCanvasToImage() {
        ...
        for (let i = 0; i < canvas.length; i++) {
            ...
            for (let j=0; j<canvas[i]._objects.length; j++){
                if(canvas[i]._objects[j].type === 'text'){
                    let canvas_text =canvas[i]._objects[j].text;

                    var new_msg = canvas_text.replace(/([#0-9]\u20E3)|[\xA9\xAE\u203C\u2047-\u2049\u2122\u2139\u3030\u303D\u3297\u3299][\uFE00-\uFEFF]?|[\u2190-\u21FF][\uFE00-\uFEFF]?|[\u2300-\u23FF][\uFE00-\uFEFF]?|[\u2460-\u24FF][\uFE00-\uFEFF]?|[\u25A0-\u25FF][\uFE00-\uFEFF]?|[\u2600-\u27BF][\uFE00-\uFEFF]?|[\u2900-\u297F][\uFE00-\uFEFF]?|[\u2B00-\u2BF0][\uFE00-\uFEFF]?|(?:\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDEFF])[\uFE00-\uFEFF]?|[\u20E3]|[\u26A0-\u3000]|\uD83E[\udd00-\uddff]|[\u00A0-\u269F]/g, '').trim();
                    canvas[i]._objects[j].text = new_msg
                }
            }
    ...
    DuplicateDesigns = () => {
        let selected_canvas = this.state.canvas[this.state.currentIndex].toJSON(['canvas_id', 'type', 'image_id', 'is_empty_image', 'image_name']);
        ...
        let user_image = this.state.userSelectedImages;

        let userSelectedImages = []; // Empty Array
        ...
        for (let i = 0; i < quantity; i++) {
            ...
            for (let j = 0; j < selected_canvas.objects.length; j++) {
                ...
                if (j === selected_canvas.objects.length - 1) {
                    ...
                    canvas[i].loadFromJSON(value, function () {
                        canvas[i].renderAll()
                    });
                }
            ...
            previewImages[i] = this.state.previewImages[this.state.currentIndex];

            userSelectedImages[i] = user_image[this.state.currentIndex];


            if (i === quantity - 1) {
                this.setStateValue(canvas, previewImages, userSelectedImages);
            }
    ...
    setStateValue = (canvas, previewImages, userSelectedImages) => {
        let user_selected_images = userSelectedImages;
        for (let i = 0; i < user_selected_images.length; i++) {
            user_selected_images[i]['image_name'] = 'image_' + i;
                ...
                <div>
                    <Container maxWidth="xl" style={{height: '100vh'}}>
                        <Grid container style={{height: '100vh'}}>
                            <Grid item xs={7} className="left_canvasGrid" style={{height: '100%'}}>
                                <div className="desktop_canvas_section">
                                                    ...
                                                    return (
                                                        <div className="slide" key={index}>
                                                            <canvas ref={(ref) => this.canvasRefs[`canvas${id}`] = ref}
                                                                    ...
                                                                    width={this.state.canvas_width} key={index}
                                                                    id={index} className="canvas_section"
                                ...
                                <div className="thumb_image_sections">
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            <img src={image.preview_url}
                                                                 ...
                                                                 className='desktop_canvas_thumnail'/>
                                                        </div>
                                                    )
                                                })
                                            }
                            ...
                            <Grid item xs={5} style={{height: '100%'}}>
                                <div>
                                    <ExpansionPanel square expanded={this.state.panel1_expanded}
                                                    onChange={() => this.toggleExpansionPanel('panel1')}
                                                    className="canvas_operation_panel">
                                                        ...
                                                        <label>
                                                            {this.state.userSelectedImages[this.state.currentIndex].is_empty_image === false ?
                                                                <img src={replaceImgIcon} alt='replace'
                                                                     className="replace_placeholder_image"/> :
                                                                <img src={uploadImgIcon} alt='upload'
                                                                     className="replace_placeholder_image"/>
                                                            }
                                                            <input type='file' id="fileUpload"
                                                                   className="fileInput replace_placeholder_input"
                                    ...
                                    <ExpansionPanel square expanded={this.state.panel2_expanded}
                                                    onChange={() => this.toggleExpansionPanel('panel2')}
                                                    className="canvas_operation_panel">
                                        <ExpansionPanelSummary aria-controls="panel2d-content" id="panel2d-header"
                                                               expandIcon={<ExpandMoreIcon/>}
                                                               style={{backgroundColor: '#fff'}}>
                                            <Typography>Edit Text</Typography>
                                        </ExpansionPanelSummary>
                                        <MuiExpansionPanelDetails style={{backgroundColor: '#ebebeb'}}>
                                            <div className="canvas_operation_form">
                                                <div className="text_input_and_color">
                                                    <input type="text" className="desktop_canvas_text"
                                                    ...
                                                    <button type="button" onClick={this.addTextToCanvas}
                                                            className="add_text_btn">Add Text
                                                    </button>
                                                    <input type="color" value={this.state.textColor} id="text-bg-color"
                                                           size="10" onChange={(e) => this.changeTextStyle(e, 'color')}
                                                           className="color_feild"/>
                                                </div>
                                                <div className="text_font_styles">
                                                    <select onChange={(e) => this.changeTextStyle(e, 'fontfamily')}
                                                            value={this.state.selected_font}
                                                            className="font_family_select">
                                                        {
                                                    ...
                                                    <select onChange={(e) => this.changeTextStyle(e, 'fontsize')}
                                                            value={this.state.selected_font_size}
                                                            className="font_size_select">
                                                        {
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onClick={() => this.applyStylesToText('', 'bold')}
                                                            style={{fontWeight: '600'}}>B
                                                    </button>
                                                    <button type="button"
                                    ...
                                    <div className="final_submission_btn_section">
                                        {/*<button type="button" className="save_and_continue_design_class" onClick={this.DuplicateDesigns}>DUPLICATE DESIGN</button>*/}
                                        <button type="button" className="save_and_continue_design_class"
                                                onClick={this.SaveAndContinue}>SAVE
                                        </button>
                                        <button type="button" className="save_and_continue_design_class"
                                                onClick={this.ResetCanvasObjects}>RESET
                                        </button>
                                        <button type="button" className="save_and_continue_design_class"
...
const LeftArrow = (props) => {
    return (
        <div className="desktop_backArrow arrow" onClick={props.goToPrevSlide}>
...
const RightArrow = (props) => {
    return (
        <div className="desktop_nextArrow arrow" onClick={props.goToNextSlide}>
