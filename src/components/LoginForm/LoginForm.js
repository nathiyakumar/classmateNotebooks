        ...
        CreateUserLoginWithEmailMutation(loginData,function (response) {
            ...
            if(response.userLoginWithEmail !== null){
                ...
                if(response.userLoginWithEmail.user.isVendor === true){
                    ...
                    scope.props.history.push('/dashboard');

                } else if(response.userLoginWithEmail.user.isAdmin === true){
                    scope.props.history.push('/admin-dashboard/orders');
                } else {
                                ...
                                this.state.is_email === false && this.state.is_mobile_number === false ?(
                                    <div>
                                        <div className="login_first_field">
                                            ...
                                            <FormInput type="text" placeholder="Enter Your Email-ID or Mobile Number"
                                        ...
                                        <div className="loginform_action">
                                            <p className="new_customer_link" onClick={() => this.props.handleFormChange('signup')}>New Customer? Register</p>
                                            <button type="button" className="continue_btn" disabled={this.state.mob_or_email.length === 0 } onClick={this.checkEmailOrMobile}><span>Continue</span><Icon
...

                            {
                                this.state.is_email?(
                                    <div className="login_second_field">
                                        ...
                                        <div className="loginform_action">
                                            <p className="new_customer_link" onClick={() => this.props.handleFormChange('signup')}>New Customer? Register</p>
                                            <button type="button" className="continue_btn" disabled={this.state.password.length === 0 } onClick={this.LoginWithEmail}><span>Continue</span><Icon
                                                className="fa fa-arrow-right " style={{fontSize: 16, margin: "-3px 10px"}}/></button>
                                        </div>
                                    </div>
                                ):null
...

                            {
                                this.state.is_mobile_number?(
                                        <div className="login_second_field_otp">
                                            ...
                                            <div className="loginform_action">
                                                <p className="new_customer_link" onClick={() => this.props.handleFormChange('signup')}>New Customer? Register</p>
                                                <button type="button" className="continue_btn" disabled={otp.length < numInputs} onClick={this.LoginWithMobileNo}><span>Continue</span><Icon
                                                    className="fa fa-arrow-right " style={{fontSize: 16, margin: "-3px 10px"}}/></button>
                                            </div>
